...
import { TourModel, TourType } from "./tour.model";
...
const getAllTours = async (query: any) => {
  ...
  const searchField = tourSearchableField.map(field => ({
  ...
  const filter = searchTerm ? { $or: searchField } : {};

  const data = await TourModel.find(filter)
    .skip(skip)
    .limit(limit);
